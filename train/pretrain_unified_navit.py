...
class TrainingArguments:
    ...
    use_wandb: bool = field(
        default=True, metadata={"help": "Whether use Weights & Biases for logging."}
    )
...
def main():
    ...
    train_loader = DataLoader(
        ...
        drop_last=True,
        prefetch_factor=(
            data_args.prefetch_factor if data_args.prefetch_factor > 1 else None
        ),
    )
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        if curr_step % training_args.log_every == 0:
            ...
            if training_args.use_wandb and dist.get_rank() == 0:
